...
class LineChart extends React.Component {
  ...
  findOne = storeTarget => {
    const data = this.props.newCustomers.data[
      this.props.dataPositions[storeTarget]
    ]
    if (data) {
      return this.props.newCustomers.data[this.props.dataPositions[storeTarget]]
        .data
    } else {
  ...
  addCircles(storeTarget, colourMap) {
    const circles = select(`.${this.props.panel} .g_${storeTarget}`)
      ...
      .attr('cy', d => this.yScale(d[this.props.newCustomers.opts.yAxisVar]))
  ...
  updateLinePlot() {
    this.linePlot = line()
      ...
      .y(d => this.yScale(d[this.props.newCustomers.opts.yAxisVar]))
  ...
  updateYAxisLabel(w, h, duration) {
    const {
      newCustomers: {
        opts: { yAxisVar, currentSports, sportTarget },
        const: { yMapInv }
      }
    } = this.props

    const getY = () => {
      if (yAxisVar === 'All Routes') {
  ...
  updateYScale(w, h) {
    const yAxisVar = this.props.newCustomers.opts.yAxisVar
  ...
  updateChart(w, h, duration = 0) {
    for (var i = 0; i < this.props.data.length; i++) {
      const id = this.props.data[i][this.props.newCustomers.const.primaryKey]
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    //   if (this.props[key] !== prevProps[key] && key !== 'children') {
    //     console.log(key)
    //   }
    // })

    const changedVar = yAxisVar != prevProps.newCustomers.opts.yAxisVar
    ...
    if (shouldDraw) {
      ...
      } else if (this.props.newCustomers.nodata && !changedLegend) {
  ...
  render() {
    ...
    const { loaded } = this.props.newCustomers
    return loaded ? (
