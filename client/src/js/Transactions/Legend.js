...
class Legend extends React.Component {
  ...
  applyVarChange() {
    const { currentStores } = this.props.transactions.opts
    for (var i = 0; i < currentStores.length; i++) {
  ...
  addLegendItem(storeTarget, col, row) {
    const {
      transactions: {
        opts: { dateRange, dateRangeAggLevel },
    ...
    if (isDefined) {
      const mapping = transactions_map[primaryKeyLabel]
        ...
        () => {
          return result
            ? this.props.dataFormat(
                result[this.props.transactions.opts.yAxisVar].toFixed(2)
              )
            : ''
        }
      )
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    const changedVar =
      this.props.transactions.opts.yAxisVar !=
      prevProps.transactions.opts.yAxisVar
    ...
    const changedNoData = nodata !== prevProps.transactions.nodata
